    ...
    <Wrapper>
      ...
      <CreateContent>
        <Distance top={0} bottom={20}>
          ...
          <TextInput placeholder="어떤 모임을 같이 하고 싶나요?" maxLength={30} />
        </Distance>
        <Distance top={0} bottom={20}>
        ...
        <Distance top={0} bottom={0}>
          ...
          {period.map((value) => (
              <>
                <Radio type="radio" name="period" id={value.id} defaultValue={value.id} onClick={radioPeriodHandler} />
                <RadioLabel htmlFor={value.id} Ischecked={checkedPeriod === value.id}>{value.value}</RadioLabel>
              </>
            ))}
          </RadioWrap>
          {checkedPeriod === 'period1' && <TextInput placeholder="정기모임을 할 약속시간을 입력해주세요." />}
          {checkedPeriod === 'period2' && <TextInput placeholder="모임을 할 약속시간과 날짜를 입력해주세요." />} {/* 캘린더로 바꾸기 */}
        </Distance>
        <Line />
        <Distance top={0} bottom={0}>
          ...
          {personnel.map((value) => (
              <>
                <Radio type="radio" name="personnel" id={value.id} defaultValue={value.id} onClick={radioPersonnelHandler} />
                <RadioLabel htmlFor={value.id} Ischecked={checkedPersonnel === value.id}>{value.value}</RadioLabel>
              </>
            ))}
            {checkedPersonnel === 'personnel2' && <TextInput width='120px' marginTop='0' marginBottom="0" placeholder="숫자만"/>} {/* 캘린더로 바꾸기 */}
          </RadioWrap>
        </Distance>
        <Line />
        <Distance top={0} bottom={0}>
          ...
          {gender.map((value) => (
              <>
                <Radio type="radio" name="gender" id={value.id} defaultValue={value.id} onClick={radioGenderHandler} />
                <RadioLabel htmlFor={value.id} Ischecked={checkedGender === value.id}>{value.value}</RadioLabel>
              </>
            ))}
          </RadioWrap>
        </Distance>
        ...
        <Distance top={0} bottom={40}>
          ...
          {age.map((value) => (
              <>
                <Radio type="radio" name="age" id={value.id} defaultValue={value.id} onClick={radioAgeHandler} />
                <RadioLabel htmlFor={value.id} Ischecked={checkedAge === value.id}>{value.value}</RadioLabel>
              </>
            ))}
          </RadioWrap>
          {checkedAge === 'age1' && (<><TextInput width='48.5%'/> ~ <TextInput width='48.5%'/></>)} {/* 텍스트 커스텀 하기 */}
        </Distance>
...
width: 120px;
padding: 0 28px;
background :url(${(props: {Ischecked:boolean}) =>props.Ischecked ? checkmark_full : checkmark_outline}) no-repeat left center / 20px;
`;
