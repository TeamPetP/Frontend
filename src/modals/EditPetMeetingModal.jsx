...
import { SearchDetailPost, EditMeetingBoard } from "../services/MeetingApi";
...
const Input = styled.input`
	display: none;
`;
function EditPetMeetingModal(props) {
	...
	async function Update() {
		if (text != "") {
			const editPostData = await EditMeetingBoard(
				user,
				modalStore.getPetMeetingEditId,
				modalStore.getPetMeetingEditPostId,
				{
					content: text,
					title: title,
					imgUrlList: image,
			...
			if (editPostData.status == 200) {
				window.location.href = `/meeting/detail?id=${modalStore.getPetMeetingEditId}`;
			}
		}
	}
	...
	return (
		<Modal
			...
			closeVisible={() => props.EditPetMeetingModalState()}
			width="1200"
		>
			<ModalWrapper>
				<Title>게시글 수정</Title>
			...
			<BackIcon
				...
				onClick={() => {
					setText("");
					setImage([]);
					props.EditPetMeetingModalState();
				}}
			/>
		</Modal>
	);
...
export default EditPetMeetingModal;
